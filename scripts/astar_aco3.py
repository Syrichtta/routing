...
# Configure logging
logging.basicConfig(level=logging.INFO, filename="aco_log.txt", filemode="w", format="%(threadName)s: %(message)s")

# ACO parameters (can be adjusted)
num_ants = 10
num_iterations = 10
...
def ant_single_iteration(G, start_node, end_node, max_path_length, pheromone_levels):
    ...
    while stack:
        ...
        if current_node == end_node:
            ...
            if path_length <= max_path_length:
                logging.info(f"Ant completed path with length: {path_length:.2f}")
                path_found = True
                return path, path_length
            else:
                logging.info(f"Path discarded: Length {path_length:.2f} exceeds max {max_path_length:.2f}")
        ...
        if unvisited_neighbors:
            ...
            if random_choice > 0.5:
                ...
                logging.info(f"Uses A* heuristic to select node {next_node}")
            else:
                ...
                logging.info(f"Uses ACO heuristic to select node {next_node}")
            ...
            if potential_path_length <= max_path_length:
                ...
                logging.info(f"Prevented from adding node {next_node} to prevent exceeding max path length")
                break
        else:
            # Backtrack if no unvisited neighbors
            logging.info(f"Stuck at node {current_node} (no unvisited neighbors). Backtracking...")
            stack.pop()  # Backtrack by removing the last node from the stack

    # If no path found
    return None, float('inf')

def ant_colony_optimization_multithreaded(G, start_node, end_node, initial_best_path=None, max_workers=None):
    ...
    for iteration in range(num_iterations):
        ...
        # Use ThreadPoolExecutor for parallel ant path finding
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Create futures for each ant
            futures = [
                executor.submit(
                    ant_single_iteration, 
...
def main():
    ...
    best_path, best_path_length, all_paths = ant_colony_optimization_multithreaded(
        G, 
        start_node, 
        end_node, 
        initial_best_path=initial_best_path, 
        max_workers=None  # Use default number of workers (typically number of CPU cores)
    )
    end_time = time.time()
